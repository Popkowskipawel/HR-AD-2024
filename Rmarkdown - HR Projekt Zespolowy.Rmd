...
Ze względu na brak zróżnicowania w kolumnach ,,EmployeeCount", "Over18" i ,,StandardHours", usuwamy je z naszego 
zbioru danych.
```{r}
...
```
Z wykresu można zauważyć większą tendencję do rotacji, pracowników w młodszym wieku.
```{r}
complete_data %>%
...
```
Powyższy wykres obrazuje wyższy poziom rotacji pracowników, mających większą odległość do dom
# Wnioskowanie
